<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CustomerTrade.aspx.cs"
    Inherits="EAPP.CRM.Web.CustomerTrade" MasterPageFile="~/MainTemplate.Master" %>
...
<asp:Content ID="content1" runat="server" ContentPlaceHolderID="contentPlaceHolderMain">
    ...
    <script type="text/javascript">
            function chkData()
            {
                var txtName  = $("<%= this.txtName.ClientID %>");
                if(!txtName || txtName.value.length < 6)
                {
                    window.alert("请输入客户来源。");
                    txtName.focus();
                    return false;
                }
                return true;
    ...
    <div id="content">
        ...
        <div id="right">
            ...
            <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" CssClass="gridview"
                Width="100%" DataKeyNames="TradeId" OnRowCancelingEdit="GridView1_RowCancelingEdit"
                OnRowDeleting="GridView1_RowDeleting" OnRowEditing="GridView1_RowEditing" OnRowUpdating="GridView1_RowUpdating">
                <Columns>
                    <asp:TemplateField HeaderText="行业名称">
                        <EditItemTemplate>
                            <asp:TextBox ID="TextBox1" runat="server" Text='<%# Eval("TradeName") %>'></asp:TextBox>
                        </EditItemTemplate>
                        <ItemTemplate>
                            <asp:Literal ID="Literal1" runat="server" Text='<%# Eval("TradeName") %>'></asp:Literal>
                        </ItemTemplate>
                        <ItemStyle Width="200" />
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="标示名称">
                        <EditItemTemplate>
                            <asp:TextBox ID="TextBox2" runat="server" Columns="30" Text='<%# Eval("FlagName") %>'
                                TextMode="MultiLine"></asp:TextBox>
                        </EditItemTemplate>
                        <ItemTemplate>
                            <asp:Literal ID="Literal2" runat="server" Text='<%# Eval("FlagName") %>'></asp:Literal>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="操作">
                        <EditItemTemplate>
                            <asp:LinkButton ID="LinkButton3" runat="server" CommandName="Update" CommandArgument='<%# Eval("TradeId") %>'>更新</asp:LinkButton>
                            &nbsp;<asp:LinkButton ID="LinkButton4" runat="server" CommandName="Cancel">取消</asp:LinkButton>
                        </EditItemTemplate>
                        <ItemTemplate>
                            <asp:LinkButton ID="LinkButton1" runat="server" CommandName="Edit" CommandArgument='<%# Eval("TradeId") %>'>编辑</asp:LinkButton>
                            &nbsp;<asp:LinkButton ID="LinkButton2" runat="server" CommandName="Delete" OnClientClick="return window.confirm('确定要删除');"
                                CommandArgument='<%# Eval("TradeId") %>'>删除</asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle Width="97" />
                    </asp:TemplateField>
                </Columns>
